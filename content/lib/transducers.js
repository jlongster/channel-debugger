...
Take.prototype.step = function(result, input) {
  // TODO: actually this needs to always return Reduced(result) if i
  // is greater than n, I see now...
  if (this.i < this.n) {
    result = this.xform.step(result, input);
    if(this.i + 1 >= this.n) {
      // Finish reducing on the same step as the final value.
...
function identity(r) {
  return {
    init: function() { return r.init(); },
    result: function(x) { return r.result(x); },
    step: function(result, x) { return r.step(result, x); }
  };
};

function seq(coll, xform) {
  if(isArray(coll)) {
    return transduce(coll, xform || identity, arrayReducer, []);
  }
  else if(isObject(coll)) {
    return transduce(coll, xform || identity, objReducer, {});
  }
  else if(fulfillsProtocol(coll, 'transformer')) {
    var transformer = getProtocolProperty(coll, 'transformer');
    return transduce(coll, xform || identity, transformer, transformer.init());
  }
  else if(fulfillsProtocol(coll, 'iterator')) {
    return new LazyTransformer(xform || identity, coll);
  }
